...
def plot_report(report_path: str | Path, output_path: str | Path | None = None) -> Path:
    ...
    concurrency = [x["concurrency"] for x in levels]
    output_path = Path(output_path) if output_path else report_path.with_suffix(".png")
    fig, axes = plt.subplots(2, 2, figsize=(14, 9))
    ax = axes[0, 0]
    ax.plot(concurrency, [x["throughput_tps"] for x in levels], "o-", label="Output TPS")
    ax.set_title("Total throughput"); ax.set_ylabel("token/s"); ax.grid(alpha=.3)
    ax = axes[0, 1]
    for key in ("p50", "p95", "p99"):
        ax.plot(concurrency, [x["ttft"][key] for x in levels], "o-", label=key)
    ...
    ax = axes[1, 0]
    for key in ("p50", "p95", "p99"):
        ax.plot(concurrency, [x["itl"][key] * 1000 for x in levels], "o-", label=key)
    ...
    if any(x["gpu"]["available"] for x in levels):
        ax.plot(concurrency, [x["gpu"]["avg_utilization_pct"] for x in levels], "o-", label="GPU avg %")
        ax.plot(concurrency, [x["gpu"]["max_vram_utilization_pct"] for x in levels], "s--", label="VRAM max %")
    ...
    fig.text(.5, .01, f"NOTICE: {saturation['reason']} Recommended concurrency: {saturation['level']} | {warning_count} warning(s) in JSON",
             ha="center", bbox={"facecolor": color, "edgecolor": "#aa5555", "pad": 6})
    fig.suptitle(f"vLLM benchmark — {report['config']['model']}")
